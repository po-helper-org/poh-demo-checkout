...
- Счётчики посещений и успешных транзакций хранятся **in-memory** в замыкании модуля `metrics.mjs`.
- Состояние **сбрасывается при рестарте сервера** или при hot-reload — все счётчики обнуляются.
- Решение **не поддерживает fork/cluster режим** — состояние локально для процесса и не распределяется между воркерами.

Эти ограничения допустимы для демо-стенда, но должны учитываться при использовании в продакшене.
